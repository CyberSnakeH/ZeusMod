...
void DrawMenu() {
    ...
    if (ImGui::BeginTable("layout", 2, ImGuiTableFlags_SizingStretchProp | ImGuiTableFlags_NoPadOuterX)) {
        ...
        DrawFeatureRow("items", "Infinite Items", "Stops inventory items from decrementing when used AND pins tool/armor durability at max (999,999).", &trainer.InfiniteItems);
        ...
        ImGui::Separator();
        ImGui::Spacing();
        ImGui::TextColored(Accent(), "Item Spawner");
        ImGui::TextDisabled("Give yourself any item from the %zu known rows. Type a filter, click a row to receive it.",
            Trainer_GiveItem_GetAllNames().size());
        ImGui::Spacing();
        {
            ...
            ImGui::InputTextWithHint("Filter", "start typing (e.g. 'wood', 'iron', 'arrow')",
                s_filterBuf, sizeof(s_filterBuf));
            ImGui::SliderInt("Count", &s_giveCount, 1, 999);
            if (s_lastGiven[0]) {
                ImGui::TextColored(s_lastOk ? Success() : Danger(),
                    "Last: %s  %s", s_lastGiven, s_lastOk ? "placed" : "REJECTED");
            }

            // Lowercase copy of the filter so comparison is case-insensitive.
            std::string filter;
            for (const char* p = s_filterBuf; *p; ++p)
                filter.push_back(static_cast<char>(tolower((unsigned char)*p)));

            ImGui::BeginChild("itemList", ImVec2(0.0f, 220.0f), true);
            const auto& names = Trainer_GiveItem_GetAllNames();
            int shown = 0;
            for (const std::string& n : names) {
                if (!filter.empty() && n.find(filter) == std::string::npos) continue;
                if (shown++ > 300) { ImGui::TextDisabled("...more results hidden, tighten filter"); break; }
                if (ImGui::Selectable(n.c_str())) {
                    bool ok = Trainer_GiveItem(n.c_str(), s_giveCount);
                    std::snprintf(s_lastGiven, sizeof(s_lastGiven), "%s x%d", n.c_str(), s_giveCount);
                    s_lastOk = ok;
                }
            }
