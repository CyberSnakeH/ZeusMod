...
std::string ResolveFNameByIndex(int32_t comparisonIndex) {
    return ResolveFNameIndex(comparisonIndex);
}

int32_t FindStructPropertyOffset(const char* structName, const char* propName) {
    if (!s_initialized || !structName || !propName) return -1;
    uintptr_t s = FindStructByName(structName);
    if (!s) s = FindClassByName(structName);   // fallback
    if (!s) { LOG_UOBJ("FindStructPropertyOffset: '%s' not found", structName); return -1; }
    // FindPropertyOffsetInClass walks this struct only; structs may inherit
    // via UStruct::Super just like classes, so do a bounded Super walk.
    int hops = 0;
    while (s && hops < 16) {
        int32_t offset = FindPropertyOffsetInClass(s, propName);
        if (offset >= 0) return offset;
        uintptr_t super = 0;
        if (!SafeRead(s + 0x40, super) || !super) break;
        s = super;
        hops++;
    }
    LOG_UOBJ("FindStructPropertyOffset: %s::%s not found (walked %d supers)",
        structName, propName, hops);
    return -1;
}

int32_t ResolveEnumValue(const char* enumName, const char* memberName) {
    if (!s_initialized || !enumName || !memberName) return -1;
    // Find the UEnum object — it lives in GObjects as a named object with
    // class "Enum". Walk by name since there can be one-shot enums without
    // dedicated classes.
    ...
    for (int32_t i = 0; i < total; ++i) {
        ...
        if (!obj) continue;
        std::string cls = GetObjectClassName(obj);
        if (cls != "Enum" && cls != "UserDefinedEnum") continue;
        std::string name = GetObjectName(obj);
        if (name == enumName) { uenum = obj; break; }
    }
    if (!uenum) { LOG_UOBJ("ResolveEnumValue: UEnum '%s' not found", enumName); return -1; }

    // UEnum::Names = TArray<TPair<FName, int64>> at +0x40 (validated via
    // `dump 0x26930D12200 0x100` → 16-byte entries {FName(8), int64(8)}).
    uintptr_t names = 0;
    int32_t   count = 0;
    if (!SafeRead(uenum + 0x40, names) || !names) return -1;
    if (!SafeRead(uenum + 0x48, count) || count <= 0 || count > 4096) return -1;

    // Each entry carries the *prefixed* FName ("EDynamicItemProperties::Durability").
    // Match either on the prefixed form or the bare member name suffix.
    std::string prefix = std::string(enumName) + "::";
    for (int32_t i = 0; i < count; ++i) {
        uintptr_t entry = names + static_cast<uintptr_t>(i) * 16;
        std::string full = ReadFNameAt(entry);
        if (full.empty()) continue;
        int64_t value = 0;
        if (!SafeRead(entry + 8, value)) continue;
        if (full == memberName) return static_cast<int32_t>(value);
        if (full.size() > prefix.size() && full.compare(0, prefix.size(), prefix) == 0
            && full.compare(prefix.size(), std::string::npos, memberName) == 0) {
            return static_cast<int32_t>(value);
        }
    }
    LOG_UOBJ("ResolveEnumValue: %s::%s not in Names TArray", enumName, memberName);
    return -1;
}
...
uintptr_t FindClassByName(const char* className) {
    ...
    for (int32_t i = 0; i < total; ++i) {
        ...
        // Accept every concrete UClass flavour UE4 uses: plain UClass,
        // Blueprint-generated classes for Actors / Pawns / GameModes,
        // Widget-Blueprint-generated classes for UMG (e.g.
        // UMG_EncumbranceBar_C), and Animation Blueprint classes. Without
        // this breadth, FindPropertyOffset silently fails for any UMG
        // widget or animation BP and callers see zero offsets downstream.
        if (clsOfObj != "Class" &&
            clsOfObj != "BlueprintGeneratedClass" &&
            clsOfObj != "WidgetBlueprintGeneratedClass" &&
            clsOfObj != "AnimBlueprintGeneratedClass") continue;
...
// Same as FindClassByName but also matches UScriptStruct. ScriptStructs share
// the UStruct base (Super @+0x40, ChildProperties @+0x50) so FField-based
// layout walks work on them identically — only this resolver needs to be
// broader than FindClassByName.
uintptr_t FindStructByName(const char* structName) {
    if (!structName || !*structName) return 0;
    int32_t total = GetObjectCount();
    if (total <= 0) return 0;
...
// === FProperty layout constants (UE 4.27) ==================================
// FField common header (FProperty inherits from this):
//   +0x00 vtable
//   +0x08 ClassPrivate (FFieldClass*)
//   +0x10 Owner (FFieldVariant — 16 bytes union)
//   +0x20 Next (FField*)
//   +0x28 NamePrivate (FName — 8 bytes)
//   +0x30 FlagsPrivate (EObjectFlags — uint32)
//
// FProperty adds:
//   +0x38 ArrayDim (int32)
//   +0x3C ElementSize (int32)
//   +0x40 PropertyFlags (uint64 — EPropertyFlags bitmask)
//   +0x48 RepIndex (uint16)
//   +0x4C Offset_Internal (int32) — byte offset within owner struct
constexpr int OFF_FFIELD_CLASSPRIVATE = 0x08;
constexpr int OFF_FPROPERTY_ELEMENTSIZE = 0x3C;
constexpr int OFF_FPROPERTY_FLAGS       = 0x40;

// EPropertyFlags — subset we care about
...
constexpr uint64_t CPF_ReferenceParm = 0x0000000008000000ULL;
constexpr uint64_t CPF_ConstParm     = 0x0000000000000002ULL;

// FFieldClass starts with an FName. This is its layout for UE 4.27:
//   +0x00 Name (FName — 8 bytes)
//   +0x08 Id    (uint64 class id)
//   +0x10 CastFlags (uint64)
//   ...
// We only need the name.
constexpr int OFF_FFIELDCLASS_NAME = 0x00;

static std::string ReadFieldClassName(uintptr_t fieldAddr) {
    uintptr_t fieldClassPtr = 0;
    if (!SafeRead(fieldAddr + OFF_FFIELD_CLASSPRIVATE, fieldClassPtr) || !fieldClassPtr) {
        return "<nocls>";
    }
    return ReadFNameAt(fieldClassPtr + OFF_FFIELDCLASS_NAME);
}

// Dump every parameter (FProperty) attached to a UFunction. Prints the
// property name, its FFieldClass (type), its offset into the param buffer,
// its element size, and which Parm-kind flags it has. This is what we need
// to build a ProcessEvent call: allocate a buffer of UFunction::ParmsSize,
// fill each param slot at its Offset_Internal, then invoke ProcessEvent.
void DumpUFunctionSignature(uintptr_t ufunctionAddr, const char* label) {
    if (!s_initialized || !ufunctionAddr) return;

    uint8_t  numParms = 0;
    uint16_t parmsSize = 0;
    uint16_t returnOff = 0;
    SafeRead(ufunctionAddr + 0xB4, numParms);
    SafeRead(ufunctionAddr + 0xB6, parmsSize);
    SafeRead(ufunctionAddr + 0xB8, returnOff);

    uintptr_t field = 0;
    SafeRead(ufunctionAddr + OFF_USTRUCT_CHILDPROPS, field);

    LOG_UOBJ("--- SIG %s @ 0x%p  NumParms=%u  ParmsSize=0x%X  ReturnOff=0x%X ---",
        label ? label : "<fn>", (void*)ufunctionAddr, numParms, parmsSize, returnOff);

    int safety = 64;
    int idx = 0;
    while (field && safety-- > 0) {
        std::string name = ReadFNameAt(field + OFF_FFIELD_NAME);
        std::string type = ReadFieldClassName(field);
        int32_t offsetInternal = -1;
        int32_t elementSize = 0;
        uint64_t flags = 0;
        SafeRead(field + OFF_FPROPERTY_OFFSET_INT, offsetInternal);
        SafeRead(field + OFF_FPROPERTY_ELEMENTSIZE, elementSize);
        SafeRead(field + OFF_FPROPERTY_FLAGS, flags);

        char flagBuf[64] = "";
        int flen = 0;
        if (flags & CPF_Parm)          flen += snprintf(flagBuf + flen, sizeof(flagBuf) - flen, "Parm ");
        if (flags & CPF_OutParm)       flen += snprintf(flagBuf + flen, sizeof(flagBuf) - flen, "Out ");
        if (flags & CPF_ReturnParm)    flen += snprintf(flagBuf + flen, sizeof(flagBuf) - flen, "Ret ");
        if (flags & CPF_ReferenceParm) flen += snprintf(flagBuf + flen, sizeof(flagBuf) - flen, "Ref ");
        if (flags & CPF_ConstParm)     flen += snprintf(flagBuf + flen, sizeof(flagBuf) - flen, "Const ");

        LOG_UOBJ("  [%d] %-30s type=%-25s off=+0x%02X size=0x%X flags=0x%llx %s",
            idx, name.c_str(), type.c_str(),
            (unsigned)offsetInternal, (unsigned)elementSize,
            (unsigned long long)flags, flagBuf);
    ...
    LOG_UOBJ("--- SIG end (%d property nodes) ---", idx);
}

// Convenience: resolve a UFunction by class+name then dump its signature.
void DumpUFunctionSignatureByName(const char* className, const char* funcName) {
    uintptr_t cls = FindClassByName(className);
    if (!cls) { LOG_UOBJ("SIG: class '%s' not found", className); return; }
    uintptr_t fn = FindFunctionInClass(cls, funcName);
    if (!fn)  { LOG_UOBJ("SIG: %s::%s not found", className, funcName); return; }
    char tag[96];
    _snprintf_s(tag, sizeof(tag), _TRUNCATE, "%s::%s", className, funcName);
    DumpUFunctionSignature(fn, tag);
}

// Enumerate DataTable UObject instances whose name contains `substring`.
// For each, print the UObject address, the RowStruct name (the UScriptStruct
// the datatable rows are typed as), and an estimate of the row count by
// probing the TMap internals.
//
// UDataTable layout (UE 4.27):
//   +0x28 UScriptStruct* RowStruct
//   +0x30 TMap<FName, uint8*> RowMap  -- 0x50 bytes
//
// TMap<K,V> is TSortableMapBase wrapping a TSet<TPair<K,V>>. TSet owns a
// TSparseArray of elements. For our purposes we just need the element count.
// TSparseArray layout:
//   +0x00 TArray<Element>     Data    (data, num, max)
//   +0x10 TBitArray            AllocationFlags
//   +0x38 int32                FirstFreeIndex
//   +0x3C int32                NumFreeIndices
// "Real" row count = Data.Num - NumFreeIndices.
constexpr int OFF_UDATATABLE_ROWSTRUCT = 0x28;
constexpr int OFF_UDATATABLE_ROWMAP    = 0x30;

void DumpDataTableInstances(const char* substring, int maxResults) {
    if (!s_initialized) return;
    const char* needleRaw = substring ? substring : "";
    std::string needle(needleRaw);
    StrLowerInPlace(needle);

    int total = GetObjectCount();
    int found = 0;
    LOG_UOBJ("=== DumpDataTableInstances('%s') ===", needleRaw);
    ...
    for (int32_t i = 0; i < total && found < maxResults; ++i) {
        ...
        uintptr_t rowStruct = 0;
        SafeRead(obj + OFF_UDATATABLE_ROWSTRUCT, rowStruct);
        std::string rowStructName = rowStruct ? GetObjectName(rowStruct) : std::string("?");

        // Probe TMap element count via its inner TSparseArray Data.Num
        // (offset within RowMap = 0x08, since TArray header is {ptr, num, max})
        int32_t dataNum = -1, freeCount = -1;
        SafeRead(obj + OFF_UDATATABLE_ROWMAP + 0x08, dataNum);
        // FreeIndices inside TSparseArray, header(+0x10 BitArray=0x28) -> FirstFree @ +0x38
        SafeRead(obj + OFF_UDATATABLE_ROWMAP + 0x3C, freeCount);
        int approxRows = (dataNum >= 0 ? dataNum : 0) - (freeCount > 0 ? freeCount : 0);

        LOG_UOBJ("  [%d] %s @ 0x%p  rowStruct=%s  ~rows=%d (dataNum=%d freeIdx=%d)",
            i, name.c_str(), (void*)obj, rowStructName.c_str(),
            approxRows, dataNum, freeCount);
        found++;
    }
    LOG_UOBJ("=== %d DataTable(s) ===", found);
}
...
// === UFunction invocation via ProcessEvent =================================
//
// Problem: ProcessEvent's vtable index varies by engine version (and compile
// options). UE 4.22-4.27 range: 65..68 typically, 67 most common on stock
// 4.27. To stay build-proof we try each candidate slot, invoke a known-safe
// UFunction with no side-effects (ItemTemplateLibrary::NumRows — returns an
// integer row count) and trust the slot whose call returns a plausible
// value without crashing.

static FnProcessEvent s_processEvent = nullptr;

FnProcessEvent ResolveProcessEvent(void* knownInstance) {
    if (s_processEvent) return s_processEvent;
    if (!s_initialized || !knownInstance) return nullptr;

    void** vtable = nullptr;
    __try {
        vtable = *reinterpret_cast<void***>(knownInstance);
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        LOG_UOBJ("ResolveProcessEvent: vtable read threw");
        return nullptr;
    }
    if (!vtable) return nullptr;

    uintptr_t libCls = FindClassByName("ItemTemplateLibrary");
    if (!libCls) {
        LOG_UOBJ("ResolveProcessEvent: ItemTemplateLibrary not found — cannot validate");
        return nullptr;
    }
    uintptr_t numRowsFn = FindFunctionInClass(libCls, "NumRows");
    if (!numRowsFn) {
        LOG_UOBJ("ResolveProcessEvent: NumRows UFunction not found");
        return nullptr;
    }

    // Canonical UE 4.27 is 67; probe a small window around it. ProcessEvent
    // lives in the engine module, not game code, but safeInstall-style
    // range validation still helps reject data pointers that happen to be
    // in the vtable slot range.
    static const int kCandidates[] = { 67, 66, 65, 68, 64, 69, 70, 71 };
    for (int idx : kCandidates) {
        uintptr_t pe = 0;
        __try {
            pe = reinterpret_cast<uintptr_t>(vtable[idx]);
        ...
        bool crashed = false;
        __try {
            FnProcessEvent candidate = reinterpret_cast<FnProcessEvent>(pe);
            candidate(knownInstance,
                      reinterpret_cast<void*>(numRowsFn),
                      paramsBuf);
            result = *reinterpret_cast<int32_t*>(paramsBuf);
        } __except (EXCEPTION_EXECUTE_HANDLER) {
            crashed = true;
        }

        if (!crashed && result > 0 && result < 100000) {
            LOG_UOBJ("ResolveProcessEvent: vtable[%d] = 0x%p (NumRows validated -> %d)",
                     idx, (void*)pe, result);
            s_processEvent = reinterpret_cast<FnProcessEvent>(pe);
            return s_processEvent;
        }
        LOG_UOBJ("ResolveProcessEvent: vtable[%d] = 0x%p rejected (crashed=%d result=%d)",
                 idx, (void*)pe, (int)crashed, result);
    }
    LOG_UOBJ("ResolveProcessEvent: no candidate validated");
    return nullptr;
}

bool CallUFunction(void* self, uintptr_t ufunction, void* paramsBuffer) {
    if (!self || !ufunction || !paramsBuffer) return false;
    if (!s_processEvent && !ResolveProcessEvent(self)) return false;
    __try {
        s_processEvent(self, reinterpret_cast<void*>(ufunction), paramsBuffer);
        return true;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        return false;
    }
